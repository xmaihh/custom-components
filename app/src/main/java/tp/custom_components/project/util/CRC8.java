...
//        System.out.println(s);
        ...
        String s = String.valueOf(calcCrc(hexStr2Bytes("AA550922D1226400BA")));
        if (s.equals("0")) {
            System.out.println(s);
        } else {
        ...
        byte c[] = {(byte) 0xAA, (byte) 0x55, (byte) 0x09, (byte) 0x22}; //, (byte) 0x0C};
        byte a = calcCrc(c);
        int b = ByteUtil.byteToInt(a);
        System.out.println(ByteUtil.byteToInt(a));
        System.out.println("计算得出CRC1： " + ByteUtil.toHex(b, 1));

        short data = 100;
        ByteBuffer bb = ByteBuffer.allocate(2);
        bb.order(ByteOrder.LITTLE_ENDIAN); //小端序 ,从左往右 左低位 右高位
        bb.putShort(data);
        byte d[] = bb.array();
        System.out.println("计算得出Data： " + ByteUtil.toHex(d));
        byte d2[] = {(byte) 0xAA, (byte) 0x55, (byte) 0x09, (byte) 0x22, (byte) 0xd1, (byte) 0x22};
        byte crc2 = calcCrc(ByteUtil.combine(d2, d));
        int e = ByteUtil.byteToInt(crc2);
        System.out.println(ByteUtil.byteToInt(crc2));
        System.out.println("计算得出CRC2： " + ByteUtil.toHex(e, 1));
    }
}
